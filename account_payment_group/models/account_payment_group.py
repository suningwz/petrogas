...
class AccountPaymentGroup(models.Model):
    ...
    def payment_print(self):
        ...
        return self.env.ref('account_payment_group.payment_group_report').report_action(self)
        # self.sent = True
        # raise ValidationError(_('Not implemented yet'))
    ...
    def post(self):
        ...
        for rec in self:
            ...
            sequence = False
            # if rec.name == '/' and rec.partner_type == 'customer':
            if rec.partner_type == 'customer':
                sequence = self.env['ir.sequence'].next_by_code('payment.group.customer')

            # if rec.name == '/' and rec.partner_type == 'supplier':
            if rec.partner_type == 'supplier':
                sequence = self.env['ir.sequence'].next_by_code('payment.group.supplier')
                # sequence = self.env['ir.sequence'].next_by_code('payment.group.supplier')

            if not sequence:
                raise ValidationError(""" Veuillez définir une séquence pour les réglements groupés""")

            rec.name = sequence
            rec.create_bank_statement_line()
    ...
    @api.multi
    def create_bank_statement_line(self):
        """ Create un bank statement line si non exitant"""
        for pay_group in self:
            for payment_id in pay_group.payment_ids.filtered(lambda x: x.journal_id.is_bank_statement and  x.move_line_ids
                                                                       and not x.move_line_ids.mapped('statement_line_id')):
                """Récupérer caisse"""
                bank_statement = self.env['account.bank.statement'].search([('journal_id', '=', payment_id.journal_id.id),
                                                                            ('state', '=', 'open'),
                                                                            ('date', '<=', payment_id.payment_date)])
                if not bank_statement:
                    raise UserError("""Ensure that a cash register is open for the day %s!!! """ % payment_id.payment_date)
                bank_statement.ensure_one()

                """Récupérer créer ligne"""
                res = {
                    'date': payment_id.payment_date,
                    'name': payment_id.name,
                    'ref': payment_id.payment_group_id.name,
                    'amount': payment_id.amount if payment_id.payment_type == 'inbound' else -1 * payment_id.amount,
                    'statement_id': bank_statement.id,
                    'partner_id': payment_id.partner_id.id,
                }

                statement_line_id = self.env['account.bank.statement.line'].create([res])
                move_id = payment_id.move_line_ids.filtered(lambda x: x.account_id == payment_id.journal_id.default_credit_account_id
                                                                      or x.account_id == payment_id.journal_id.default_debit_account_id)
                move_id.ensure_one()
                statement_line_id.journal_entry_ids = [(4, move_id.id)]
