...
class AccountPayment(models.Model):
    ...
    def create(self, vals):
        ...
        if create_payment_group:
            ...
            if self.env.context.get('payment_group_id', False):
                payment_group = self.env.context.get('payment_group_id')
            else:
                payment_group = self.env['account.payment.group'].create({
                'company_id': company_id,
                'partner_type': vals.get('partner_type'),
                'partner_id': vals.get('partner_id'),
                'payment_date': vals.get('payment_date', fields.Date.context_today(self)),
