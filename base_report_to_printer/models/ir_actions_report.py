...
class IrActionsReport(models.Model):
    ...
    print_limit = fields.Boolean(string="Print Limit", help="If set, limit only one print if number_of_print exist in object", default=False)
    no_limit_print_id = fields.Many2one('res.groups', 'No Limit Group', help='Personn in this group will authorizated to repeat print document')
    number_of_copy = fields.Integer('Number of print', help="how many document to print requested.", default=1)
    ...
    def render_qweb_pdf(self, res_ids=None, data=None):
        ...
        """
        if res_ids and self.number_of_copy:
            number_of_copy = self.number_of_copy if hasattr(self, 'number_of_copy') else 1
            res_ids *= number_of_copy
            # res_ids *=  self.number_of_copy
    ...
    def _get_rendering_context(self, docids, data):
        ...
        data.update({
            'number_of_copy': self.number_of_copy,
        })
        return data
