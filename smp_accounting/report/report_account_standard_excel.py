...
class StandardReportXlsx(models.AbstractModel):
    ...
    #
    #             all_lines = wizard._sql_get_line_for_report(type_l=('4_total',))
    #             # all_lines = wizard._sql_get_line_for_report(type_l=('0_init', '1_init_line', '4_total',))
    #             # for group_by in data['group_by_data']['ids']:
    ...
    #                     sheet.write(i, 0, line.get('code', ''))
    #                     sheet.write(i, 1, line.get('name', ''))
    #                     sheet.write(i, 2, line.get('initial_balance', ''), currency_format)
    #                     sheet.write(i, 3, line.get('debit', ''), currency_format)
    #                     sheet.write(i, 4, line.get('credit', ''), currency_format)
    #                     sheet.write(i, 5, line.get('balance', ''), currency_format)
    #                     # sheet.write(i, , line.get('view', ''), currency_format)
    #                 row = i
    #
    #                 for j, h in enumerate(head):
    #                     sheet.set_column(j, j, h['larg'])
