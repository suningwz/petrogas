...
class SaleAnalysisReport(models.TransientModel):
    ...
    def excel_write(self):
        ...
        writer = pd.ExcelWriter(data_buffer, engine='xlsxwriter')
        # writer = pd.ExcelWriter(data_buffer, engine='xlsxwriter')
        df.to_excel(writer, index=False, sheet_name='Liste des fiches')
        ...
        writer.save()
        file = base64.encodestring(data_buffer.getvalue())
        data_buffer.close()
