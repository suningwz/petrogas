...
class StockMoveLine(models.Model):
    ...
    inter_uom_factor = fields.Float(string="Convesion factor", digits=(10, 4), related='move_id.inter_uom_factor')
    ...
    @api.one
    @api.depends('product_id', 'product_uom_id', 'product_uom_qty', 'inter_uom_factor')
    def _compute_product_qty(self):
        if self.product_id.uom_id.category_id != self.product_uom_id.category_id and \
                self.product_id.uom_po_id.category_id == self.product_uom_id.category_id:
    ...
    def _action_done(self):
        ...
        for ml in self:
            ...
            qty_done = float_round(ml.qty_done, precision_rounding=ml.product_uom_id.rounding, rounding_method='HALF-UP')
        ...
        for ml in self - ml_to_delete:
            if ml.product_id.type == 'product':
                ...
                if ml.product_id.uom_id.category_id != ml.product_uom_id.category_id \
                        and ml.product_uom_id.category_id == ml.product_id.uom_po_id.category_id:
                    product_po_qty = ml.product_uom_id._compute_quantity(ml.qty_done, ml.product_id.uom_po_id)
                    quantity = product_po_qty * ml.inter_uom_factor
                else:
