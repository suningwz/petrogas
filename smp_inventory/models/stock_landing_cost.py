...
class LandingCost(models.Model):
    ...
    cost_lines = fields.One2many(
        'stock.landing.cost.lines', 'cost_id', 'Cost Lines',
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('stock.landing.cost')
        return super(LandingCost, self).create(vals)

    @api.multi
    def unlink(self):
        self.button_cancel()
        return super(LandingCost, self).unlink()

    @api.multi
    def _track_subtype(self, init_values):
        if 'state' in init_values and self.state == 'done':
            return 'stock_landed_costs.mt_stock_landed_cost_open'
        return super(LandingCost, self)._track_subtype(init_values)
...
class LandingCostLine(models.Model):
    _name = 'stock.landing.cost.lines'
    _description = 'Stock Landed Cost Line'

    name = fields.Char('Description')
    purchase_order_id = fields.Many2one('puchase.order', 'Landed Cost', required=True, ondelete='cascade')
...
class PurchaseCharges(models.Model):
    _name = 'purchase.charges'
    _description = 'Purchase Charges'

    name = fields.Char('Description', compute='_compute_name', store=True)
    # cost_id = fields.Many2one('stock.landing.cost', 'Landed Cost', ondelete='cascade', required=True)
    cost_line_id = fields.Many2one('stock.landing.cost.lines', 'Cost Line', readonly=True)
    move_id = fields.Many2one('stock.move', 'Stock Move', readonly=True)
    product_id = fields.Many2one('product.product', 'Product', required=True)
    quantity = fields.Float('Quantity', default=1.0, digits=0, required=True)
    weight = fields.Float('Weight', default=1.0, digits=dp.get_precision('Stock Weight'))
    volume = fields.Float('Volume', default=1.0)
